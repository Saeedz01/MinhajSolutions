...
const TechItem = ({ tech }) => {
  ...
  return (
    <div className="bg-white border-2 border-blue-100 rounded-xl flex items-center justify-center hover:shadow-lg hover:border-black transition-all duration-300 hover:scale-105 aspect-square w-full h-auto sm:h-40 sm:w-auto lg:h-36 lg:w-56 p-4 sm:p-6 lg:p-8 cursor-pointer">
      {!imgError[tech.name] ? (
        <img
          src={tech.logo}
          alt={tech.name}
          className="w-16 sm:w-20 lg:w-20 h-16 sm:h-20 lg:h-20 object-contain"
          onError={() => setImgError(prev => ({ ...prev, [tech.name]: true }))}
        />
      ) : (
        <div className="text-center text-gray-700 font-semibold text-sm sm:text-base lg:text-sm">
          {tech.name}
        </div>
      )}
    </div>
...
const TechStackSection = ({ slug }) => {
  ...
  const getTechStackKey = (slug) => {
    if (!slug) return 'SoftwareEnterpriseTechnologies';
